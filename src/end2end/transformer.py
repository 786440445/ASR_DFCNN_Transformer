...
def multihead_attention_decoder(queries, keys,
                        d_model=None, num_heads=8, dropout_rate=0,
                        is_training=True, causality=False, scope="multihead_attention", reuse=None):
    """
    多头注意力机制
...
def feedforward_with_decoder(inputs, num_units, scope="positionwise_ffnn", reuse=None):
    '''Point-wise feed forward net.
...
def multihead_attention_in_transformer(queries, keys,
                        d_model=None, num_heads=8, dropout_rate=0,
                        is_training=True, causality=False, scope="multihead_attention", reuse=None):
    """
    多头注意力机制
...
def feedforward_in_transformer(inputs, num_units, scope="positionwise_ffnn", reuse=None):
    '''Point-wise feed forward net.
...
def residual_norm_connection(input, outputs):
    # Residual connection 参差链接，将query加起来。
    outputs += input
    # Normalize layerNorm
...
def shift_right(input, pad=1):
    """Shift input tensor right to create decoder input. '1' denotes <S>"""
